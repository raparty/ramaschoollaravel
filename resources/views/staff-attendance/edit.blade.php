...
@section('title', 'Edit Staff Attendance - School ERP')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2>Edit Staff Attendance</h2>
        <p class="text-muted mb-0">Modify staff attendance records</p>
    </div>
...
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('staff-attendance.edit') }}" class="row g-3">
            <div class="col-md-5">
...
@if($date && $staffMembers->count() > 0)
    <!-- Attendance Form -->
    <form method="POST" action="{{ route('staff-attendance.update') }}" id="attendanceForm">
        @csrf
        @method('PUT')
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @foreach($staffMembers as $staff)
                                @php
                                    $existing = $attendance[$staff->id] ?? null;
...
@elseif($date && $staffMembers->count() == 0)
    <div class="card">
