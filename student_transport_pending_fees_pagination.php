...
require_once("config/config.inc.php");
	...
	*/
	 ...
	 $query = "SELECT COUNT(*) as num FROM $tbl_name  where (registration_no not in (SELECT registration_no FROM student_transport_fees_detail where session='".$_SESSION['session']."' and fees_term='".$_POST['fees_term']."') and class='".$_POST['class']."' )";
         if($_POST['stream']!="")
						{
	...
	/* Get data. */
      ...
      $sql10 = "SELECT * FROM $tbl_name where (registration_no not in (SELECT registration_no FROM student_transport_fees_detail where session='".$_SESSION['session']."' and fees_term='".$_POST['fees_term']."') and class='".$_POST['class']."' )";
	 if($_POST['stream']!="")
						{
