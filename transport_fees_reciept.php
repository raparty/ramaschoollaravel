...
<title>Transport Fees Reciept </title>
...
color:#ffffff;
 ...
 <?php
    ...
    if( $_SESSION['fees_term']!="all")
	{
		...
		header('location:transport_fees_reciept_byterm.php?registration_no='.$_SESSION['registration_no']."&&fees_term=".$_SESSION['fees_term']);
		die();
		}
			  ...
			  $registration_no=$_SESSION['registration_no'];
			   ...
			   $sql="SELECT * FROM school_detail";
					...
					$reciept_no='';
					 $select_fees_detail123=mysql_query("select * from student_transport_fees_detail where registration_no='".$rows['registration_no']."' and session='".$_SESSION['session']."' order by student_fees_id	 ASC");
...
<table width="100%" id="student1">
<tr>
<th>Fees Month</th><th>Fees Amount</th><th>Deposite Date</th><th>Session</th>

</tr>
<?php 
                            $registration_no=$rows['registration_no'];
				            $select_fees_detail=mysql_query("select * from student_transport_fees_detail where registration_no='".$rows['registration_no']."' and session='".$_SESSION['session']."' order by fees_term ASC");
					...
					?>		
<tr>
<td><strong><?php echo $month['month_name'];?></strong></td><td><?php echo $row['fees_amount'];?></td><td><?php echo date('d-m-Y',strtotime($row['deposit_date'])); ?></td><td><?php echo $row['session'];?></td>
</tr>
<?php } ?>
<!--<tr>
...
<tr>
<td><?php echo $sql_transport_row['cost'];?></td>
<td><?php echo $deposit_amount[0];?></td>
<td><?php echo $pending_amount=$sql_transport_row['cost']-$deposit_amount[0];?></td>
...
<table width="100%" id="student1">
<tr >
<td colspan="6" id="hidebutton" style="display:block;"> <a href="student_transport_fees.php"  style="text-decoration:none;"><input type="button" name="search" value="Back" class="btn_small btn_orange"></a>  <a href="#" onclick="return printdata()"  style="text-decoration:none; margin-left:50px;"><input type="button" name="search" value="Print" class="btn_small btn_orange"></a></td>
</tr>
